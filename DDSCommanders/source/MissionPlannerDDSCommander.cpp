...
void MissionPlannerDDSCommander::receivedFinderMessageListResult(const FinderMessageList &findermessages) {
	...
	for (int i=0;i<findermessages.messages2d.length(); i++) {
		...
		vec2d.vec.push_back(FinderResult2D(findermessages.cameraid, msg.u, msg.v, msg.scale, msg.angle));
	}
	ptr->Operate(vec2d);
	...
	for (int i=0;i<findermessages.messages3d.length(); i++) {
		...
		vec3d.vec.push_back(FinderResult3D(findermessages.cameraid, msg.x, msg.y, msg.z, msg.ang1, msg.ang2, msg.ang3));
	}
	ptr->Operate(vec3d);
}
