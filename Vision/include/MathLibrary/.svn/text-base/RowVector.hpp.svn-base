...
RowVector<size, T>::RowVector(const Matrix<1, size, T> &matrix)
{
 for (int c = 1; c <= size; c++)
  this->d_element[0][c-1] = matrix(1, c);
}

template<int size, class T>
RowVector<size, T>::RowVector(const RowVector<size, T> &rowVector)
{
 for (int c = 0; c < size; c++)
  this->d_element[0][c] = rowVector.d_element[0][c];
}
...
RowVector<size, T> &RowVector<size, T>::operator=(const VectorBase<T> &v)
{
 ...
 {
  this->d_element[0][i-1] = v.getElement(i);
 }
 return *this;
}
