...
class ODESolverRK4
{
 public:
  ...
  ODESolverRK4(double samplingPeriod, const T &initialValue);
  ...
  inline void setSamplingPeriod(double samplingPeriod) { d_samplingPeriod = samplingPeriod; }
  ...
  virtual void reset(const T &resetToValue);
   ...
   //  resetToValue  The value to which the solver
   //		     output is reset to.
...
T ODESolverRK4<T>::stepSolve()
{
 ...
 d_previousOutput = d_previousOutput + ((1.0/6.0) * (k1 + (2 * (k2+k3)) + k4));
